        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach ($projects as $project)
                <a href="/projects/{{ $project->slug }}">
                    <x-bubble class="max-w-full flex flex-col xs:flex-row h-full xs:aspect-[2/1] [&]:p-0 xs:[&:has(.the-image:hover)_.the-text]:w-0 min-w-0">
                        <div class="the-text order-2 xs:order-first w-full xs:w-1/2  flex flex-col min-h-0 transition-[width] duration-600 shrink-0">
                            <h3 class="mx-6 mt-4 text-white mb-1">{{ $project->name }}</h3>
